...
import cn.com.realer.core.support.DataType;
import cn.com.realer.core.support.exception.DataParseException;
...
public final class TypeParseUtil {
	...
	public static final Object convert(Object value, String type, String format) {
		...
		}else if(value.toString().equals("null")||value.toString().equals("")){
			return null;
		}
	...
	private static java.util.Date str2Date(String str, String type, String format, Locale locale){
		try{
			return new java.util.Date(Long.parseLong(str));
		}catch (Exception e0){}
		if (format == null || format.length() == 0) {
			try {
				return new java.util.Date(Date.valueOf(str).getTime());//sql date  参数为 4年2月2日
			} catch (Exception e) {}
			try {
				DateFormat df = null;
				if (locale != null)
		...
		try {
			...
			throw new DataParseException(String.format(message, str, type), e1);
		}
	...
	private static Object string2Object(Object value, String type, String format, Locale locale) {
		...
		try {
			if ("String".equalsIgnoreCase(type) || DataType.STRING.equalsIgnoreCase(type)) {
				...
				nf.setMaximumFractionDigits(0);
				Number tempNum = nf.parse(str.replaceAll(",", ""));
				return new Short(tempNum.longValue()+"");
			} else if ("java.sql.Date".equalsIgnoreCase(type)||"Date".equalsIgnoreCase(type) || DataType.DATE.equalsIgnoreCase(type)) {
				java.util.Date date=str2Date(str,type,format,locale);
